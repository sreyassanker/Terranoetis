...
| **DuckdbAnalyticsPanel** | — | In-browser DuckDB-WASM SQL workbench: loads 7 live data layers as queryable tables, runs real SQL (filter/join/aggregate), result grid + sample queries |
...
| `server/observability/` | Logging (Pino), metrics (Prometheus), error classes, OpenTelemetry, Sentry error tracking |
...
- **Authentication:** `/api/auth/login`, `/api/auth/dev-login`, `/api/auth/refresh` (JWT refresh)
...
- **Observability:** `/api/health` (10-component status incl. Redis), `/api/ready`, `/api/live`, `/api/metrics`, `/api/observability/traces`, `/api/observability/metrics`, `/api/admin/metrics` (admin)
- **API Docs:** `/api/openapi.json` (OpenAPI 3.0.3 spec), `/api/docs` (Swagger UI)
...
3. **Computation** — the peer-reviewed equation (split across `equationParts/PART1–7`, composed into `EQUATION_ENGINE` in `engine.ts`, ~14,100 lines of pure functions across the parts)
...
- Launch Replay
- World Radio Tuner
- **DuckDB Spatial SQL** (in-browser SQL over live data layers)
- Intelligence Panel
...
**Toolbar toggles:** Compute Demo (one-click Austin LST), DuckDB Spatial SQL, Sensor Styles 1–6 (Normal/CRT/NVG/FLIR/Noir/Snow), Photorealistic Globe, plus layer toggles for Street Traffic, World Radio, Bikeshare, Camera Viewsheds, Detection Overlay, 3D Aircraft Models.
...
- Rate limiting (per-IP and per-user; public data endpoints capped at 300/min per IP)
- SSRF guard for outbound URL validation
- Input validation with Zod schemas
- Audit logging for sensitive operations
- Tenant isolation for multi-user scenarios (ownership enforced on chats/monitor rules/scheduled tasks; chat upsert rejects cross-user overwrite; feedback scoped per user)
...
- Sentry error tracking (SENTRY_DSN in `.env`; captures 5xx + unhandled errors only, 4xx excluded)
- Docker HEALTHCHECK uses `curl -f` (true HTTP-status check) on all services
...
- **Error Tracking:** SENTRY_DSN (Sentry project DSN, optional — unset = no-op)
