...
export const CORE_PAGES = [
  // ═════════════════════════════════ DOCS HOME ══════════════════════════
  {
    ...
    sections: [
      ...
      { kind: 'metrics', items: [
        ...
        verifiedFact('Equation-engine tests', '1,653 unit tests passed (141 files, 22.85 s wall) with <code>npm run test:unit</code>', srcRaw('measured')),
        ...
        verifiedFact('Hazard physics', 'All 7 kernels executed locally in this review; representative + boundary inputs, conservation gates, convergence and benchmark suites re-run — see <a href="methodology.html#evidence-runs">Evidence runs</a>', srcRaw('measured')),
      ...
      { kind: 'table', caption: 'Top-level layout', cols: ['Path', 'Contents'], rows: [
        ...
        ['<code>kaggle-kernels/</code>', 'Seven Python simulation kernels + per-kernel metadata; runtime output goes to <code>results/&lt;jobId&gt;/</code> (gitignored)'],
        ['<code>docs/</code>', 'This site: generated HTML pages (source: scripts/docs/), Markdown prose of record, manifest and sitemap'],
        ['<code>e2e/</code>', 'Playwright specs (7 files; 15 test declarations, 14 registered by default — 1 gated behind <code>LULC_HEAVY=1</code>)'],
  ...
  // ════════════════════════════ GETTING STARTED ═══════════════════════════
  {
    ...
    tags: ['<span class="tag tag-measured">ALL STEPS EXECUTED</span>'],
    sections: [
      ...
      { kind: 'table', caption: 'Feature availability without credentials (verified boot + code paths)', cols: ['Feature', 'Without keys', 'Evidence'], rows: [
        ...
        ['Flood / tsunami / volcano / landslide sims', 'Disabled until a Kaggle token exists at ~/.kaggle/kaggle.json', src(SR, 'Kaggle pipeline requires token') + ' ' + src('docs/site/markdown/DEPLOYMENT.md', '162-173')],
  ...
  // ════════════════════════════ ARCHITECTURE ══════════════════════════════
  {
    ...
    sections: [
      ...
      { kind: 'table', caption: 'Middleware/service order in server/index.ts (line cites are the app.use/startup sequence; boot observed on port 3399)', cols: ['Order', 'Component', 'Detail', 'Source'], rows: [
        ...
        ['6', 'global /api guard', 'public allow-list: health/metrics/config/OpenAPI + rate-limited read-only data routes (300/min/IP) + unthrottled public non-data routes (tiles, pulse, share-token reads); everything else requires JWT', src('server/index.ts', '373-487')],
        ['7', 'routers', 'foundation-models, fork, pulse, /api/kaggle, analytical-models, openapi', src('server/index.ts', '489-494')],
        ['8', 'DynamicToolRegistry (DB-persisted; 210 tools loaded at boot) + Omninet init + local GGUF spawn', 'llama-server :11436 when model file present', src('server/index.ts', '513-817')],
      ...
      { kind: 'p', html: `LOC per directory and each module’s purpose are tabulated in the prose document (<a href="narratives/BACKEND.html">BACKEND.md</a>), verified against the tree. Highlights of interest to researchers: <code>analytical-models/</code> (23,866 LOC), <code>kaggle/</code> (3,105), <code>cognition/</code> (2,760), <code>sentinel/</code> (3,191), <code>memoryV2/</code> (2,091).` },
  ...
  // ════════════════════════ ANALYTICAL ENGINE ════════════════════════════
  {
    ...
    sections: [
      ...
      { kind: 'table', cols: ['Claim', 'Measured', 'Method'], rows: [
        ['150 engines, ids 1–150', '150 unique ids, no gaps', 'programmatic key enumeration of EQUATION_ENGINE via tsx import ' + src('server/analytical-models/engine.ts', '1-20,170')],
      ...
      { kind: 'table', caption: 'Sample computations executed in-process (no network)', cols: ['Tool', 'Inputs', 'Measured result', 'Check'], rows: [
      ...
      { kind: 'p', html: `Topic lists per domain and individual citations: <a href="../narratives/MODELS.html">MODELS.md</a> (corrected). Reproduction: <code>npx tsx -e "import {computeEquation} from './server/analytical-models/engine.ts'; console.log(computeEquation(19,{a:7,b:1,M:6}))"</code>` },
  ...
  // ════════════════════════ LIVE DATA & MONITORING ═══════════════════════
  {
    ...
    sections: [
      ...
      { kind: 'table', cols: ['Metric', 'Value', 'Method/source'], rows: [
        ['Registered API metadata entries', '82', 'regex count <code>category:</code> in server/apiMetadata.ts'],
  ...
  // ════════════════════════ GLOBE VISUALIZATION ══════════════════════════
  {
    ...
    sections: [
      ...
      { kind: 'p', html: `Component tables and line counts: <a href="../narratives/FRONTEND.html">FRONTEND.md</a> (corrected).` },
    ],
  },
  ...
  // ════════════════════════ PLATFORM SERVICES ════════════════════════════
  {
    ...
    sections: [
      ...
      { kind: 'table', cols: ['Signal', 'Implementation', 'Source'], rows: [
        ...
        ['Tracing/errors', 'in-house OpenTelemetry-style tracing init + Sentry opt-in via SENTRY_DSN', src('server/index.ts', '14450-14457')],
