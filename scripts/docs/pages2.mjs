...
export const PAGES2 = [
  // ════════════════════════════ REFERENCE: API ═══════════════════════════
  {
    ...
    sections: [
      ...
      { kind: 'metrics', items: [
        ['Route registrations', '398 handler registrations (<code>(app|*Router).get/post/put/delete/patch(</code>) under server/ — grep count 2026-09-09; API.md headline “360+” is consistent'],
  ...
  // ════════════════════════════ METHODOLOGY ══════════════════════════════
  {
    ...
    sections: [
      ...
      { kind: 'table', caption: 'The code was treated as authoritative in every case; sources of truth updated 2026-09-09.', cols: ['#', 'Claim', 'Verified correction', 'Fixed in'], rows: [
        ...
        ['C-7', '“Playwright browser tests (7 specs, 12 tests)” (README)', '7 spec files; 15 <code>test()</code> declarations at line start, 14 registered by default per <code>npx playwright test --list</code> (“Total: 14 tests in 7 files”; the one extra land-cover test self-skips unless <code>LULC_HEAVY=1</code>). An earlier fix-note here stated “38 declarations” — that count came from a naive <code>grep "test("</code> that also matched regex <code>.test(</code> calls; corrected by direct enumeration 2026-09-09.', 'README.md · documentation hub'],
