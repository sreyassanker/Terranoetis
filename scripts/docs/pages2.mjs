...
export const PAGES2 = [
  ...
  // ════════════════════════ REFERENCE: SIMULATION API ════════════════════
  {
    ...
    tags: [tagMeasured('HTTP transcript from a live server boot')],
    sections: [
      ...
      { kind: 'table', cols: ['Method & path', 'Behaviour', 'Measured / source'], rows: [
        ...
        ['GET /api/kaggle/simulate/:id/grid/:name', 'raw .npy or JSON (JSON capped at 5M elements; 413 above; .npy uncapped); name sanitised to [A-Za-z0-9_-', src(RT, '263-307') + ' <span class="tag tag-measured">296,648-byte npy observed</span>'],
      ...
      { kind: 'p', html: 'Local and Kaggle runners converge on one layout: <code>kaggle-kernels/results/&lt;jobId&gt;/metadata.json</code> + <code>*.npy</code> grids, “so the SSE stream, grid and GeoTIFF endpoints work unchanged” ' + src(SR, '126-141') + '. Field dtypes/units are tabulated on each capability page; the <code>kaggle-kernels/results/&lt;jobId&gt;/</code> tree is populated at runtime (<code>metadata.json</code> + <code>*.npy</code>) and is gitignored — the repository ships no archived example artifacts.' },
  ...
  // ════════════════════════════ METHODOLOGY ══════════════════════════════
  {
    ...
    tags: ['<span class="tag tag-measured">VERIFICATION PASS COMPLETED</span>'],
    sections: [
      ...
      { kind: 'table', caption: 'The code was treated as authoritative in every case; sources of truth updated in the second verification pass.', cols: ['#', 'Claim', 'Verified correction', 'Fixed in'], rows: [
        ...
        ['C-7', '“Playwright browser tests (7 specs, 12 tests)” (README)', '7 spec files; 15 <code>test()</code> declarations at line start, 14 registered by default per <code>npx playwright test --list</code> (“Total: 14 tests in 7 files”; the one extra land-cover test self-skips unless <code>LULC_HEAVY=1</code>). An earlier fix-note here stated “38 declarations” — that count came from a naive <code>grep "test("</code> that also matched regex <code>.test(</code> calls; corrected by direct enumeration.', 'README.md · documentation hub'],
        ...
        ['C-11', 'Second verification pass (re-executed kernels, suites, boot; line-by-line claim audit)', 'Nine fixes: e2e suite counts “38 declarations” (naive grep artifact) → 15 declarations / 14 registered; handler count 371 (literal <code>app|router</code> only) → 398 across all router identifiers; “~100 provider keys” → 103 variables (62 key/token/secret); “archived example artifacts in <code>kaggle-kernels/results/</code>” → directory is empty and gitignored; error shape → two coexisting forms (string from handlers, <code>{error:{code,message}}</code> from central middleware); public-route wording → tiles/pulse/shared are public and unthrottled; phantom “vault” module removed from <code>routes/</code> inventory; “OpenTelemetry” relabelled as in-house OpenTelemetry-style tracing (official SDK imports are stubbed); Pages deployment paragraph → <code>docs/site/</code> via <code>deploy-docs.yml</code>; landing prerequisites badge “PYTHON 3.12+” → “PYTHON 3 + NumPy” (nothing enforces 3.12).', 'README.md · SECURITY.md · CONTRIBUTING.md · API.md · BACKEND.md · ARCHITECTURE.md · DEPLOYMENT.md · FRONTEND.md · MODELS.md · getting-started.md · capability pages · this site'],
      ]},
  ...
  // ════════════════════════════ DEPLOYMENT ═══════════════════════════════
  {
    ...
    sections: [
      ...
      { kind: 'table', cols: ['Variable', 'Behaviour'], rows: [
        ...
        ['103 variables in <code>.env.example</code> (62 named *KEY/*TOKEN/*SECRET)', 'missing key ⇒ that feature disables; the 3 local sims + analytical engine unaffected'],
        ...
        ['<code>LOCAL_SIM_TIMEOUT_MS</code>', 'local kernel spawn timeout, default 120,000 ms ' + src(SR, '136')],
      ...
      { kind: 'p', html: '<code>ci.yml</code> runs 10 jobs (typecheck, lint, unit, integration, build, Playwright, Docker build, coverage, audit, summary). <code>docs.yml</code> adds the documentation gate: build-drift check + quality-gate on pushes/PRs to <code>main</code>/<code>develop</code> touching <code>docs/site/**</code>, <code>scripts/docs/**</code>, or the root markdown files. <code>deploy-docs.yml</code> publishes <code>docs/site/</code> to GitHub Pages via <code>upload-pages-artifact</code> (workflow deploy; also manually dispatchable), with <code>.nojekyll</code> so every file is delivered byte-exact.' },
  ...
  // ════════════════════════════ GOVERNANCE ══════════════════════════════
  {
    ...
    sections: [
      ...
      { kind: 'list', items: [
        'Plain, precise, scientific prose. Present tense for behaviour (“the kernel raises…”), past tense for measurements (“1,653 tests passed”); the machine-readable <code>docs-last-reviewed</code> meta records the verification date, not the body text.',
        'No marketing superlatives and no decorative adjectives. The gate rejects a banned-vocabulary list (<code>world-class · cutting-edge · best-in-class · state-of-the-art · revolutionary · unparalleled · game-changing · industry-leading · unprecedented · next-generation · ultimate solution · seamless · blazing-fast</code>); the adjectives appear only beside measured numbers.',
      ...
      { kind: 'table', cols: ['Label', 'Meaning', 'Rule'], rows: [
        [tagMeasured(), 'produced by executing the code in this repository', 'must be reproducible from the page’s own command block'],
...

  {
    ...
    sections: [
      ...
      { kind: 'table', cols: ['Control', 'Verified detail', 'Source'], rows: [
        ['Secrets in repo', 'runtime <code>.env</code> is gitignored; Kaggle credentials read only from <code>~/.kaggle</code>', src('docs/site/markdown/DEPLOYMENT.md', '173') + ' ' + src('.gitignore', '.env')],
