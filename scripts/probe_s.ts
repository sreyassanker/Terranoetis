...
async function main(){
  for (const [lat,lon,label] of [[39.7,-86.0,'Indiana'],[41.5,-93.6,'Iowa'],[27.5,-81.5,'Florida']]) {
    const t0=Date.now();
    ...
    catch(e:any){ console.log(label, ((Date.now()-t0)/1000).toFixed(1)+'s', 'ERR:', e.message.slice(0,60)); }
  }
}
main();
