...
describe('KnowledgeGraph', () => {
  it('creates entity and retrieves it', () => {
    ...
    const row = db.prepare("SELECT * FROM knowledge_entities WHERE name='Japan'").get() as any;
  ...
  it('finds similar entities by embedding', () => {
    ...
    const rows = db.prepare("SELECT id, name, embedding FROM knowledge_entities WHERE embedding IS NOT NULL").all() as any[];
    const scored = rows.map(r => ({
      id: r.id, name: r.name,
      similarity: cosineSim(queryEmb, new Float32Array(r.embedding.buffer)),
  ...
  it('returns entity/relation counts', () => {
    ...
    const aId = (db.prepare("SELECT id FROM knowledge_entities WHERE name='A'").get() as any).id;
    const bId = (db.prepare("SELECT id FROM knowledge_entities WHERE name='B'").get() as any).id;
    db.prepare("INSERT INTO knowledge_relations (source_id, target_id, relation_type) VALUES (?, ?, 'connects')").run(aId, bId);
    const ec = (db.prepare("SELECT COUNT(*) as c FROM knowledge_entities").get() as any).c;
    const rc = (db.prepare("SELECT COUNT(*) as c FROM knowledge_relations").get() as any).c;
    expect(ec).toBe(2);
    expect(rc).toBe(1);
  });
});
...
describe('Predictor', () => {
  it('stores historical patterns', () => {
    ...
    const row = db.prepare("SELECT * FROM historical_patterns WHERE outcome='drought'").get() as any;
    ...
    const row2 = db.prepare("SELECT occurrences FROM historical_patterns WHERE outcome='drought'").get() as any;
  ...
  it('stores prediction log', () => {
    ...
    const row = db.prepare("SELECT * FROM prediction_log WHERE hazard_type='earthquake'").get() as any;
...
describe('Job queue', () => {
  it('creates and reads jobs', () => {
    ...
    const row = db.prepare("SELECT * FROM job_queue WHERE type='test_type'").get() as any;
    expect(row).toBeTruthy();
  ...
  it('respects priority ordering', () => {
    ...
    const high = db.prepare("SELECT id FROM job_queue WHERE type='task' AND status='pending' ORDER BY priority DESC LIMIT 1").get() as any;
